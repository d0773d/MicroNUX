...
readonly WORK_DIR="${MICRONUX_M6_NETWORK_WORKDIR:-${HOME}/.cache/micronux/m6-network}"
...
readonly DEFCONFIG="micronux_esp32p4_c6_defconfig"
readonly JOBS="${MICRONUX_JOBS:-$(nproc)}"
readonly OUTPUT_DIR="${WORK_DIR}/output-${BUILDROOT_VERSION}"
readonly ARTIFACT_DIR="${REPO_DIR}/out/m6-network"
...
if ((${#missing_tools[@]})); then
	printf 'Missing M6 network host tools: %s\n' "${missing_tools[*]}" >&2
	exit 1
fi
...
if ((image_size > LINUX_PARTITION_SIZE)); then
	printf 'M6 network Image is too large for the Linux partition: %s > %s bytes\n' \
...
(
	...
	sha256sum Image esp32p4-micronux.dtb metadata.bin rootfs.cpio \
		micronux-selftest micronux-exec-child micronux-netctl > SHA256SUMS
)

printf 'M6 network build complete: %s\n' "${ARTIFACT_DIR}"
