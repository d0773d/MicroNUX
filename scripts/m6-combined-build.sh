...
readonly WORK_DIR="${MICRONUX_M6_COMBINED_WORKDIR:-${HOME}/.cache/micronux/m6-combined}"
...
if ((${#missing_tools[@]})); then
	printf 'Missing M6 combined host tools: %s\n' "${missing_tools[*]}" >&2
	exit 1
fi
...
if ((image_size > LINUX_PARTITION_SIZE)); then
	printf 'M6 combined Image is too large for the Linux partition: %s > %s bytes\n' \
