...
readonly WORK_DIR="${MICRONUX_M6_WORKDIR:-${HOME}/.cache/micronux/m6}"
...
readonly DEFCONFIG="micronux_esp32p4_storage_defconfig"
readonly JOBS="${MICRONUX_JOBS:-$(nproc)}"
readonly OUTPUT_DIR="${WORK_DIR}/output-${BUILDROOT_VERSION}"
readonly ARTIFACT_DIR="${REPO_DIR}/out/m6"
...
if ((${#missing_tools[@]})); then
	printf 'Missing M6 host tools: %s\n' "${missing_tools[*]}" >&2
	exit 1
fi
...
(
	...
	sha256sum Image esp32p4-micronux.dtb metadata.bin rootfs.cpio \
		micronux-selftest micronux-exec-child micronux-storage-test > SHA256SUMS
)

printf 'M6 build complete: %s\n' "${ARTIFACT_DIR}"
