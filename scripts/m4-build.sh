...
readonly WORK_DIR="${MICRONUX_M4_WORKDIR:-${HOME}/.cache/micronux/m4}"
...
readonly DEFCONFIG="micronux_esp32p4_shell_defconfig"
readonly JOBS="${MICRONUX_JOBS:-$(nproc)}"
readonly OUTPUT_DIR="${WORK_DIR}/output-${BUILDROOT_VERSION}"
readonly ARTIFACT_DIR="${REPO_DIR}/out/m4"
...
if ((${#missing_tools[@]})); then
	printf 'Missing M4 host tools: %s\n' "${missing_tools[*]}" >&2
	exit 1
fi
