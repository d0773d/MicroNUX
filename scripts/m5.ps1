...
if (-not $SkipLinuxBuild) {
    & wsl.exe -- env MICRONUX_JOBS=16 bash "$wslRepoPath/scripts/m5-build.sh"
    if ($LASTEXITCODE -ne 0) {
        throw "MicroNUX M5 Linux build failed."
    }
}

$artifactPath = Join-Path $repoPath "out\m5"
...
$execChildPath = Join-Path $artifactPath "micronux-exec-child"
foreach ($artifact in @($imagePath, $dtbPath, $metadataPath, $selftestPath, $execChildPath)) {
    if (-not (Test-Path -LiteralPath $artifact)) {
        throw "Missing M5 artifact: $artifact"
    }
}

$idfPath = "C:\esp\v6.0.1\esp-idf"
$loaderPath = Join-Path $repoPath "loader"
$buildPath = Join-Path $repoPath "build\m5"
...
if (-not $SkipLoaderBuild) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "M5 loader configure failed."
    }
    & ninja -C $buildPath -j 16
    if ($LASTEXITCODE -ne 0) {
        throw "M5 loader build failed."
    }
}
...
if (-not $SkipFlash) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "M5 Linux payload flash failed."
    }
}

& $idfPython (Join-Path $PSScriptRoot "m5-test.py") `
    --port $Port --boots $Boots --artifact-dir $artifactPath
if ($LASTEXITCODE -ne 0) {
    throw "MicroNUX M5 hardware test failed."
}
