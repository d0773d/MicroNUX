...
readonly WORK_DIR="${MICRONUX_M3_WORKDIR:-${HOME}/.cache/micronux/m3}"
...
readonly DEFCONFIG="micronux_esp32p4_nommu_defconfig"
readonly JOBS="${MICRONUX_JOBS:-$(nproc)}"
readonly OUTPUT_DIR="${WORK_DIR}/output-${BUILDROOT_VERSION}"
readonly ARTIFACT_DIR="${REPO_DIR}/out/m3"
...
if ((${#missing_tools[@]})); then
    printf 'Missing M3 host tools: %s\n' "${missing_tools[*]}" >&2
    exit 1
fi
...
(
    cd "${ARTIFACT_DIR}"
    sha256sum Image esp32p4-micronux.dtb metadata.bin rootfs.cpio > SHA256SUMS
)

printf 'M3 build complete: %s\n' "${ARTIFACT_DIR}"
