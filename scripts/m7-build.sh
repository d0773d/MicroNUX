...
readonly BUILD_CONTRACT_FILE="${OUTPUT_DIR}/.micronux-m7-linux-contract"
...
build_contract="$({
	...
	(
		...
		git ls-files -z --cached --others --exclude-standard -- \
			...
			sort -z |
			xargs -0 sha256sum
		sha256sum scripts/m3-pack.py scripts/m7-build.sh \
...
if ! grep -Fq \
	...
	printf 'M7 USB console output is not fail-open when COM14 is closed.\n' >&2
	exit 1
fi
mmc_slot_init_block="$(sed -n '/^static int dw_mci_init_slots(/,/^}/p' \
	"${MMC_DRIVER}")"
mmc_slot_init_compact="$(tr '\n\t' '  ' <<<"${mmc_slot_init_block}" | tr -s ' ')"
if ! grep -Fq 'if (!host->num_slots || (host->num_slots != ARRAY_SIZE(host->slots) && !of_property_read_bool(host->dev->of_node, "espressif,allow-single-slot")))' \
	<<<"${mmc_slot_init_compact}"; then
	printf 'M7 kernel is missing isolated-slot support.\n' >&2
	exit 1
fi
...
for gdma_diagnostic_marker in \
	...
	'expected=00000001:0000000f:0a020001:023f7fe2:0000000f:023f7fe2:0000000f:001fff8f:001fff8f:chen\[3:0\]=0'; do
	if ! grep -Eq "${gdma_diagnostic_marker}" "${DISPLAY_DRIVER}"; then
		printf 'M7 kernel is missing patch36 GDMA diagnostic: %s\n' \
			"${gdma_diagnostic_marker}" >&2
		exit 1
	fi
done
...
for preparation_marker in \
	...
	if ! grep -Fq "${preparation_marker}" "${DISPLAY_DRIVER}"; then
		printf 'M7 kernel is missing continuous preparation evidence: %s\n' \
...
# 0x02000000/0x0000000f/0x001ffe80 on documented fields.  These source gates
# do not change M7's legacy ABI-v2 handoff contract.
for gdma_mask_marker in \
	...
	if ! grep -Eq "${gdma_mask_marker}" "${DISPLAY_DRIVER}"; then
		printf 'M7 kernel is missing GDMA semantic-mask marker: %s\n' \
			"${gdma_mask_marker}" >&2
		exit 1
	fi
done
...
for gdma_configure_marker in \
	...
	if ! grep -Fq "${gdma_configure_marker}" <<<"${gdma_configure_compact}"; then
		printf 'M7 GDMA configure gate is missing semantic check: %s\n' \
...
for gdma_runtime_marker in \
	...
	if ! grep -Fq "${gdma_runtime_marker}" <<<"${gdma_runtime_compact}"; then
		printf 'M7 GDMA runtime gate is missing semantic check: %s\n' \
...
# only and must not participate in native runtime acceptance.  These source
# gates do not change M7's legacy ABI-v2 handoff contract.
...
for programmed_video_marker in \
	...
	if ! grep -Fq "${programmed_video_marker}" \
		...
		printf 'M7 kernel is missing programmed video-policy check: %s\n' \
...
if grep -Fq 'DSI_HOST_NATIVE_VIDEO_POLICY_ACT' "${DISPLAY_DRIVER}" ||
	...
	printf 'M7 native runtime acceptance still depends on the DSI active mirror.\n' >&2
	exit 1
fi
...
if ! grep -Fq "${ACTIVE_SNAPSHOT_READ}" \
	...
	! grep -Fq 'snapshot->host_vid, snapshot->host_active,' \
		"${DISPLAY_DRIVER}"; then
	printf 'M7 kernel is missing active-mirror snapshot evidence.\n' >&2
	exit 1
fi
for active_component_marker in \
	...
	if ! grep -Fq "${active_component_marker}" \
		...
		printf 'M7 kernel is missing active-mirror component diagnostic: %s\n' \
...
for gdma_teardown_marker in \
	...
	if ! grep -Fq "${gdma_teardown_marker}" <<<"${gdma_teardown_compact}"; then
		printf 'M7 GDMA teardown gate is missing semantic check: %s\n' \
...
for gdma_semantic_block in \
	...
	if grep -Eq '(^|[ (])(st_ena0|st_ena1|sig_ena0|sig_ena1|common_st_ena|common_sig_ena)[[:space:]]*(==|!=)' \
		...
		printf 'M7 kernel retained an exact-full-word GDMA enable comparison.\n' >&2
		exit 1
	fi
done
...
if ! grep -Fq "${NATIVE_DARK_I2C_RELEASE_POLICY}" \
	...
	printf 'M7 kernel is missing the narrow dark-initialization I2C release policy.\n' >&2
	exit 1
fi
...
for scanout_log_marker in \
	...
	if ! grep -Fq "${scanout_log_marker}" \
		...
		printf 'M7 kernel is missing scanout failure raw-field diagnostic: %s\n' \
...
for scanout_phase_marker in \
	...
	if ! grep -Fq "${scanout_phase_marker}" \
		...
		printf 'M7 kernel is missing scanout qualification phase diagnostic: %s\n' \
...
if [[ -z "${native_dma_arm_block}" ]]; then
	printf 'M7 kernel is missing the native DMA arm helper.\n' >&2
	exit 1
fi
for native_dma_pre_enable_marker in \
	...
	if ! grep -Fq "${native_dma_pre_enable_marker}" \
		...
		printf 'M7 native DMA arm gate is missing pre-enable transaction check: %s\n' \
...
if [[ "${native_dma_enable_write_count}" -ne 1 ]] ||
	...
	printf 'M7 native DMA arm gate is missing the bounded post-enable safety check.\n' >&2
	exit 1
fi
if grep -Eq 'readl\([^)]*(DMA_LLI_CTRL_HI|DW_GDMA_CH_(CFG_LO|CFG_HI|LLP))' \
	...
	grep -Eq '(ctrl_hi|cfg_lo|cfg_hi|llp|llp_hi)[[:space:]]*(==|!=)' \
		<<<"${native_dma_post_enable}"; then
	printf 'M7 native DMA arm gate reads or compares hardware-owned descriptor/LLP state after enable.\n' >&2
	exit 1
fi
...
if [[ "${native_dma_post_enable_readl_count}" -ne 5 ]]; then
	printf 'M7 native DMA arm gate permits extra post-enable register reads.\n' >&2
	exit 1
fi
...
if ! grep -Fq 'micronux_native_dma_arm_frame(dsi, dsi->front_index, &snapshot)' \
	...
	printf 'M7 kernel did not retain the checked non-sleeping initial DMA arm path.\n' >&2
	exit 1
fi
...
if [[ "${native_dma_snapshot_count}" -ne 3 ]] ||
	...
	printf 'M7 IRQ DMA arm path can capture a diagnostic snapshot with NULL.\n' >&2
	exit 1
fi
...
# These source gates do not change M7's legacy ABI-v2 handoff contract.
for patch41_constant_marker in \
	...
	if ! grep -Eq "${patch41_constant_marker}" "${DISPLAY_DRIVER}"; then
		printf 'M7 kernel is missing Patch41 constant: %s\n' \
...
for patch41_bridge_marker in \
	...
	if ! grep -Fq "${patch41_bridge_marker}" \
		<<<"${native_dpi_bridge_compact}"; then
		printf 'M7 kernel is missing Patch41 bridge programming: %s\n' \
...
if [[ "$(grep -cF 'writel(DSI_BRG_NATIVE_DPI_RSV_DATA,' \
	...
	printf 'M7 black filler is not programmed exactly once before bridge enable.\n' >&2
	exit 1
fi
for patch41_runtime_marker in \
	...
	if ! grep -Fq "${patch41_runtime_marker}" \
		...
		printf 'M7 kernel runtime policy is missing Patch41 check: %s\n' \
...
if ! grep -Fq 'writel(DSI_BRG_NATIVE_MISC_ON, dsi->bridge + DSI_BRG_DPI_MISC_CONFIG);' \
	...
	printf 'M7 kernel is missing Patch41 runtime/snapshot bridge evidence.\n' >&2
	exit 1
fi
...
if [[ -z "${native_dma_fast_block}" ]]; then
	printf 'M7 kernel is missing the Patch41 fast recurring rearm helper.\n' >&2
	exit 1
fi
for patch41_fast_marker in \
	...
	if ! grep -Fq "${patch41_fast_marker}" \
		...
		printf 'M7 fast recurring rearm is missing exact operation: %s\n' \
...
if [[ "${native_dma_fast_writel_count}" -ne 4 ]] ||
	...
	printf 'M7 fast recurring rearm is not the exact four-write/read-free transaction.\n' >&2
	exit 1
fi
...
if [[ "${native_dma_irq_lock_count}" -ne 1 ]] ||
	...
	printf 'M7 recurring fast rearm is not uniquely called under state_lock.\n' >&2
	exit 1
fi
...
for patch41_capture_marker in \
	...
	if ! grep -Fq "${patch41_capture_marker}" \
		...
		printf 'M7 locked telemetry copy is missing field: %s\n' \
...
for patch41_telemetry_marker in \
	...
	if ! grep -Fq "${patch41_telemetry_marker}" "${DISPLAY_DRIVER}"; then
		printf 'M7 kernel is missing Patch41 telemetry state: %s\n' \
...
for patch41_irq_telemetry_marker in \
	...
	if ! grep -Fq "${patch41_irq_telemetry_marker}" \
		...
		printf 'M7 IRQ path is missing passive Patch41 telemetry: %s\n' \
...
if ! grep -Fq 'if (!(status0 & DW_GDMA_INT_DMA_TFR_DONE)) {' \
	...
	printf 'M7 IRQ completion mask is not fixed to transfer-done.\n' >&2
	exit 1
fi
...
if [[ "$(grep -cF 'DSI_BRG_FIFO_FLOW_STATUS' \
	...
	printf 'M7 IRQ FIFO telemetry is not sampled once and committed only on completion.\n' >&2
	exit 1
fi
if [[ "$(grep -cF 'DSI_BRG_FIFO_FLOW_STATUS' \
	...
	printf 'M7 watchdog FIFO telemetry is not a single ABI-v3-only sample.\n' >&2
	exit 1
fi
for patch41_reset_marker in \
	...
	if ! grep -Fq "${patch41_reset_marker}" \
		...
		printf 'M7 telemetry reset is missing exact state: %s\n' \
...
if ! grep -Fq 'spin_lock_irqsave(&dsi->state_lock, flags);' \
	...
	printf 'M7 telemetry reset is not locked at start and immediately before publication.\n' >&2
	exit 1
fi
...
for patch41_sysfs_block in \
	...
	for patch41_sysfs_marker in \
		...
		if ! grep -Fq "${patch41_sysfs_marker}" \
			...
			printf 'M7 sysfs telemetry is missing exact field: %s\n' \
	...
	if ! grep -Fq 'spin_lock_irqsave(&dsi->state_lock, flags);' \
		...
		printf 'M7 sysfs telemetry is not copied under state_lock.\n' >&2
		exit 1
	fi
done
# End Patch41 fast-rearm/black-filler/passive-telemetry gate.

# Patch42 starts the visible-runtime telemetry epoch at the final dark health
...
for patch42_epoch_marker in \
	...
	*"${patch42_epoch_marker}"*)
		...
		;;
	*)
		printf 'M7 kernel is missing or reorders the Patch42 final-dark telemetry epoch: %s\n' \
...
for patch43_marker in \
	...
	if ! grep -Fq "${patch43_marker}" <<<"${fault_work_compact}"; then
		printf 'M7 kernel is missing Patch43 fault evidence ordering: %s\n' \
			"${patch43_marker}" >&2
		exit 1
	fi
done
for patch43_marker in \
	...
	if ! grep -Fq "${patch43_marker}" <<<"${display_driver_compact}"; then
		printf 'M7 kernel is missing Patch43 bounded fault evidence: %s\n' \
			"${patch43_marker}" >&2
		exit 1
	fi
done
...
for patch44_marker in \
	...
	if ! grep -Fq "${patch44_marker}" <<<"${display_driver_compact}"; then
		printf 'M7 kernel is missing Patch44 bandwidth profile: %s\n' \
			"${patch44_marker}" >&2
		exit 1
	fi
done
...
if ! grep -Fq 'handoff->pixel_clock_hz != 80000000' \
	...
	printf 'M7 kernel Patch44 changed the legacy ABI2 display profile.\n' >&2
	exit 1
fi

# Patch46 validates the future ABI3 reload transaction without changing the
# shared one-shot runtime or M7's ABI2 behavior.
for patch46_constant in \
	...
	if ! grep -Eq "${patch46_constant}" "${DISPLAY_DRIVER}"; then
		printf 'M7 kernel is missing Patch46 constant: %s\n' \
			"${patch46_constant}" >&2
		exit 1
	fi
done
...
for patch46_reload_marker in \
	...
	if ! grep -Fq "${patch46_reload_marker}" \
		...
		printf 'M7 kernel is missing Patch46 reload transaction: %s\n' \
...
if ! grep -q 'DSI_HOST_FRAME_BTA_ACK_EN' \
	...
	printf 'M7 kernel is missing the YamUI-compatible frame-ACK policy.\n' >&2
	exit 1
fi
if ! grep -q 'DSI_HOST_LPCLK_CTRL' \
	...
	"${KERNEL_DIR}/drivers/video/fbdev/esp32p4-dsi.c"; then
	printf 'M7 kernel is missing the explicit DSI link policy.\n' >&2
	exit 1
fi
for m92_marker in \
	...
	'micronux_dma_arm_frame' \
	'micronux_initialize_pool_guards' \
	'micronux_pool_guards_valid' \
	...
	'INIT_DELAYED_WORK.*render_work' \
	'MICRONUX:M9.2:CLAIM state=ready abi=2 owner=linux' \
	'timing=20/20/40:4/10/30 buffers=3 dma=ch%u:irq-rearm config=0000000f:0a020001' \
	'frame-ack=disabled clock=auto lp=enabled backlight-gate=enabled' \
	'guards=pool-monitored'; do
	if ! grep -Eq "${m92_marker}" \
		"${KERNEL_DIR}/drivers/video/fbdev/esp32p4-dsi.c"; then
		printf 'M7 kernel is missing M9.2 display marker: %s\n' \
			"${m92_marker}" >&2
		exit 1
	fi
done
...
dt_compact="$(tr '\n\t' '  ' <<<"${dt_source}" | tr -s ' ')"
if grep -Fq 'espressif,allow-single-slot;' <<<"${dt_compact}" ||
	grep -Fq 'slot@0 { reg = <0x00>; status = "disabled";' \
		<<<"${dt_compact}"; then
	printf 'M7 DTB unexpectedly inherited the M9 microSD isolation profile.\n' >&2
	exit 1
fi
if ! grep -q 'display-pool@49300000' <<<"${dt_source}" ||
	...
	printf 'M7 DTB is missing the protected 4 MiB display pool.\n' >&2
	exit 1
fi
if ! grep -q 'espressif,required-handoff-abi = <0x02>;' \
	<<<"${display_node}" ||
	! grep -q 'reg-names = "dsi-host", "dsi-bridge", "gdma", "i2c";' \
	<<<"${display_node_compact}"; then
	printf 'M7 DTB did not retain the legacy ABI-v2 display contract.\n' >&2
	exit 1
fi
if grep -q 'firmware-name' <<<"${display_node}" ||
	grep -Eq '0x500a6000|0x500d6000|0x500e0000|0x500e1000|0x500e6000|0x50115000|0x5012d000' \
	<<<"${display_node_compact}"; then
	printf 'M7 DTB unexpectedly contains the M9 native firmware or cold-MMIO contract.\n' >&2
	exit 1
fi
