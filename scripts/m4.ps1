...
if (-not $SkipLinuxBuild) {
    & wsl.exe -- env MICRONUX_JOBS=16 bash "$wslRepoPath/scripts/m4-build.sh"
    if ($LASTEXITCODE -ne 0) {
        throw "MicroNUX M4 Linux build failed."
    }
}

$artifactPath = Join-Path $repoPath "out\m4"
...
foreach ($artifact in @($imagePath, $dtbPath, $metadataPath)) {
    if (-not (Test-Path -LiteralPath $artifact)) {
        throw "Missing M4 artifact: $artifact"
    }
}

$idfPath = "C:\esp\v6.0.1\esp-idf"
$loaderPath = Join-Path $repoPath "loader"
$buildPath = Join-Path $repoPath "build\m4"
...
if (-not $SkipLoaderBuild) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "M4 loader configure failed."
    }
    & ninja -C $buildPath -j 16
    if ($LASTEXITCODE -ne 0) {
        throw "M4 loader build failed."
    }
}
...
if (-not $SkipFlash) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "M4 Linux payload flash failed."
    }
}

& $idfPython (Join-Path $PSScriptRoot "m4-test.py") `
    --port $Port --boots $Boots --artifact-dir $artifactPath
if ($LASTEXITCODE -ne 0) {
    throw "MicroNUX M4 hardware test failed."
}
